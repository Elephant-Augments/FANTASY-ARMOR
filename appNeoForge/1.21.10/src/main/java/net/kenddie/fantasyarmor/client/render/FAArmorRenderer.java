...
public class FAArmorRenderer<T extends FAArmorItem, R extends HumanoidRenderState & GeoRenderState>
    ...
    public static <R extends HumanoidRenderState & GeoRenderState> void applyExtraBonesStatic(R renderState, BakedGeoModel bakedModel) {
        ...
        GeoBone epicFightCape = bakedModel.getBone("armorCapeEpicFight").orElse(null);
        ...
        switch (slot) {
            case HEAD -> {
                ...
                setHiddenSafe(epicFightCape, true);
                setHiddenSafe(frontCape, true);
                setHiddenSafe(leftLegCloth, true);
                setHiddenSafe(rightLegCloth, true);
            }
            case CHEST -> {
                setHiddenSafe(cape, !(showCape && !useEpicFightCape));
                setHiddenSafe(epicFightCape, !(showCape && useEpicFightCape));
        ...
        // Epic fight cape positioning
        if (epicFightCape != null && !epicFightCape.isHidden()) {
            ModelPart bodyPart = baseModel.body;
            epicFightCape.updatePosition(bodyPart.x, bodyPart.y, bodyPart.z);
        }
    ...
    void renderExtraBonesStatic(
        ...
        renderExtraBone(renderer, renderState, poseStack, bakedModel, buffer, cameraState, packedLight, packedOverlay, renderColor, slot, forceRenderEvenIfDescendant, "armorCapeEpicFight");
